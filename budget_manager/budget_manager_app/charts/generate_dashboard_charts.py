    ...
    def generate_line_chart(form, expenses, incomes):
        if form.is_valid():
            ...
            if expenses.exists() or incomes.exists():
                ...
                expense_trace = go.Scatter(x=expense_dates, y=expense_amounts, mode='lines+markers', name='Expenses',
                                           line=dict(color='#FA7070'))
                income_trace = go.Scatter(x=income_dates, y=income_amounts, mode='lines+markers', name='Incomes',
                                          line=dict(color='#557C55'))

                data = [expense_trace, income_trace]
                graph = go.Figure(data)

                title = f"Income and Expense Line chart"
    ...
    def generate_percentage_bar_chart(form, expenses, incomes):
        if form.is_valid():
            ...
            if total > 0:
                ...
                title = f"Percentage of Incomes and Expenses"
                fig.update_layout(title=title, xaxis=dict(title='Category'), yaxis=dict(title='Percentage'))
